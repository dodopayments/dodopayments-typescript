...
const remoteStainlessHandler = async (
  ...
  DODO_PAYMENTS_API_KEY: requireValue(readEnv('DODO_PAYMENTS_API_KEY') ?? client.bearerToken, 'set DODO_PAYMENTS_API_KEY environment variable or provide bearerToken client option'),
  DODO_PAYMENTS_WEBHOOK_KEY: readEnv('DODO_PAYMENTS_WEBHOOK_KEY') ?? client.webhookKey ?? undefined,
  DODO_PAYMENTS_BASE_URL: readEnv('DODO_PAYMENTS_BASE_URL') ?? readEnv('DODO_PAYMENTS_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
};
