...
export function dynamicTools(endpoints: Endpoint[]): Endpoint[] {
  ...
  const getEndpointTool = {
    ...
    tool: {
      ...
      description:
        'Get the schema for an endpoint in the Dodo Payments Node API. You can use the schema returned by this tool to invoke an endpoint with the `invoke_api_endpoint` tool.',
